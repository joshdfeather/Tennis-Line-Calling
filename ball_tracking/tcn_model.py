...
MODEL_PATH = "/Users/joshfeather/Documents/ball_tracking_protoyping/trajectory_models/tennis_trajectory_model_tcn.pth"
RETRAIN = False
FREQUENCY_SCALER = 4
...
class TrajectoryDataset(Dataset):
    def __init__(self, num_samples=50000, error_margin=0.06, frequency_scaler=FREQUENCY_SCALER):
        self.data = []
...
class TCN_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.tcn(x)
        x = self.output_layer(x)  # → (batch_size, output_size, seq_len)
        return x.transpose(1, 2)  # → (batch_size, seq_len, output_size)
