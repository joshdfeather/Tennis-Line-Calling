...
MODEL_PATH = "/Users/joshfeather/Documents/ball_tracking_protoyping/trajectory_models/tennis_trajectory_model_lstm.pth"
RETRAIN = True
FREQUENCY_SCALER = 4
...
class TrajectoryDataset(Dataset):
    def __init__(self, num_samples=10000, error_margin=0.06, frequency_scaler=FREQUENCY_SCALER):
        self.data = []
...
class LSTM_Model(nn.Module):
    def __init__(self, input_size=3, hidden_size=64, num_layers=2):
        ...
        self.fc = nn.Linear(hidden_size, input_size)  # Map hidden state to output trajectory
    def forward(self, x):
        lstm_out, _ = self.lstm(x)  # Process entire sequence in one pass
        output = self.fc(lstm_out)  # Map LSTM outputs to predicted trajectory
        return output
...
def train_model(model, dataset, num_epochs=25, lr=0.001):
...
def test_model(model, error_margin=0.06):
    ...
    with torch.no_grad():
        ...
        ax.plot(predicted_trajectory[:, 0], predicted_trajectory[:, 1], predicted_trajectory[:, 2],
                label="Predicted Trajectory", color="r", linestyle="--", alpha=0.7)
        ...
        ax.set_title("Actual vs Erroneous vs Predicted 3D Trajectory")
        plt.legend()
        plt.show()
