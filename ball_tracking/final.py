...
class BiLSTM_Model(nn.Module):
    ...
    def forward(self, x):
        out, _ = self.bilstm(x)  # Output: (batch, seq_len, hidden_size * 2)
        return self.fc(out)     # (batch, seq_len, input_size)
...
def test_model(model, error_margin=0.06):
    model.eval()
    with torch.no_grad():
        input_file="full_shot_test.txt"
        raw_data = np.loadtxt(input_file, delimiter=",")
        erroneous_trajectory = raw_data
        ...
        ax.plot(erroneous_trajectory[:, 0], erroneous_trajectory[:, 1], erroneous_trajectory[:, 2],
        ...
        ax.set_title("Actual vs Predicted 3D Trajectory for Bi-LSTM")
        plt.legend()
        plt.show()
